...
{
    key_value_pair(const key_value_pair& p) : first(p.first) //key_value_pair的拷贝构造函数
    {
        second = p.second;
    }

    key_value_pair(const Key& key, const T& t) : first(key) //key_value_pair的构造函数
    {
        second = t;
    }
...
protected:
    ...
    list <key_value_pair<const Key, T> > *buckets; //链表指针buckets
    ...
    void check_for_expansion() //re-hashing 扩展buckets空间
    {
...
public:
    ...
    hash_map()
    {
        ...
        length = DEFAULT_SIZE;
        load_factor = 0;
        buckets = new list<key_value_pair<const Key, T> >[DEFAULT_SIZE]; //创立一个存放链表的数组，头地址赋给buckets
    } // default constructor
    ...
    protected:
        ...
        hash_list_itr list_itr;
        list<key_value_pair<const Key, T> > *buckets; //buckets为链表指针
    ...
    pair<iterator, bool> insert(const key_value_pair<const Key, T>& x)
    {
        ...
        old_itr = find(key);
        if (old_itr != end()) //没有找到相同key的
            return pair <iterator, bool>(old_itr, false);
        int address = hash(key) % length;
        buckets[address].push_back(x); //在下表为address的链表中加入x
        count++;
        check_for_expansion();
    ...
    {
        int address = hash(key) % length; //根据key取下标
        iterator itr;
        itr.list_itr = std::find(buckets[address].begin(), 
            buckets[address].end(),
            key_value_pair<const Key, T>(key, T())); //这个find是std中的，寻找第一个匹配的值
        if (itr.list_itr == buckets[address].end()) //没找到，返回end
            return end();
        //找到了，将itr指向那个链表，返回itr
        itr.buckets = buckets; 
        itr.index = address;
        itr.length = length;
        return itr;
